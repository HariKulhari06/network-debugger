package com.hari.networkdebugger.storage

import android.content.Context
import com.hari.networkdebugger.core.config.StorageConfig
...
import com.hari.networkdebugger.storage.body.BodyFileStorage
import com.hari.networkdebugger.storage.db.NetworkEventDatabase
import com.hari.networkdebugger.storage.db.NetworkEventEntity
import com.hari.networkdebugger.storage.mapper.EntityMapper.toDomain
import com.hari.networkdebugger.storage.mapper.EntityMapper.toEntity
...
class RoomNetworkEventStore(
    ...
    private val database by lazy { NetworkEventDatabase.getInstance(context) }
    private val dao by lazy { database.networkEventDao() }
    private val bodyStorage by lazy { BodyFileStorage(context) }

    override suspend fun insert(event: NetworkEvent): Unit = withContext(Dispatchers.IO) {
    ...
    override suspend fun update(event: NetworkEvent): Unit = withContext(Dispatchers.IO) {
    ...
    override suspend fun get(id: String): NetworkEvent? = withContext(Dispatchers.IO) {
    ...
    override suspend fun search(query: String): List<NetworkEvent> = withContext(Dispatchers.IO) {
    ...
    private fun mapEntityToDomain(entity: NetworkEventEntity): NetworkEvent {
        ...
        val reqSize = entity.requestBodySize ?: reqContent?.toByteArray()?.size?.toLong() ?: 0L
        ...
        val resSize = entity.responseBodySize ?: resContent?.toByteArray()?.size?.toLong() ?: 0L
